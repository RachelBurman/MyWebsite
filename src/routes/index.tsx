...
// ─── Sections (content-page header hints) ─────────────────────────────────────
...
function ParchmentPopup({ objId, verbId, onClose }: {
  ...
  const showContent = obj.mode === 'parchment' && !!obj.contentKey
  ...
  return (
    <div className="parch-veil"
      ...
      <div className="parch-scroll">
        ...
        {showContent && (
          <div className="parch-body"><PanelContent id={obj.contentKey!} /></div>
        )}
        {!showContent && obj.mode === 'parchment' && (
          <p className="parch-no-content">
            — perhaps try <em>Look at</em> or <em>Open</em> if you want to see what's inside.
          </p>
        )}
      </div>
    </div>
...
// ─── Cabin home ───────────────────────────────────────────────────────────────
function CabinHome({ onNavigate }: { onNavigate: (id: string) => void }) {
  ...
  function handleHit(id: string) {
    const obj = CABIN_OBJECTS.find(o => o.id === id)
    if (!obj) return
    if (obj.mode === 'fullpage') {
      onNavigate(obj.contentKey!)
    } else {
